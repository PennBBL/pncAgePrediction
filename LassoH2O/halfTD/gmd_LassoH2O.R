...
td_F <- df_F[df_F$goassessDxpmr7 == "TD",]
train <- sample(1:nrow(td_F), nrow(td_F)/2, replace=FALSE)

x_td_train_F <- td_F[train, xvars]
x_td_test_F <- td_F[-train, xvars]
x_td_train_F <- as.matrix(x_td_train_F)
x_td_test_F <- as.matrix(x_td_test_F)
y_td_train_F <- td_F[train, yvar]
y_td_test_F <- td_F[-train, yvar]
...
cormat <- round(cor(x_td_train_F),2)
...
# Build models
#H20
xy_td_train_F <- cbind(x_td_train_F, y_td_train_F)
colnames(xy_td_train_F)[119] <- "ageAtScan1"
xy_td_test_F <- cbind(x_td_test_F, y_td_test_F)
colnames(xy_td_test_F)[119] <- "ageAtScan1"
xy_ps_F <- cbind(x_ps_F, y_ps_F)
colnames(xy_ps_F)[119] <- "ageAtScan1"
h2o_td_train_F <- as.h2o(xy_td_train_F)
h2o_td_test_F <- as.h2o(xy_td_test_F)
h2o_ps_F <- as.h2o(xy_ps_F)
mod <- h2o.automl(x = xvars, y = "ageAtScan1", training_frame = h2o_td_train_F, max_runtime_secs = 30)
ML_td_test_F <- as.matrix(h2o.predict(mod@leader,h2o_td_test_F))
ML_ps_F <- as.matrix(h2o.predict(mod@leader,h2o_ps_F))

#Lasso
mod_td_F <- glmnet(x_td_train_F, y_td_train_F, family=c("gaussian"))
pdf(NULL)
...
cv.out_F <- cv.glmnet(x_td_train_F, y_td_train_F, alpha=1)
pdf(NULL)
dev.control(displaylist="enable")
plot(cv.out_F)
...
# predict td
pred_td_test_F <- predict(mod_td_F,s=bestlam_1se,newx=x_td_test_F)
...
df_td_test_F <- data.frame(x_td_test_F)
df_td_test_F$real_age <- y_td_test_F/12
df_td_test_F$pred_age_lasso <- pred_td_test_F/12
df_td_test_F$pred_age_h2o <- ML_td_test_F/12

# Plot Percent Predicted to be over 18 in each two-year age bins, split by diagnosis
df_ps_F$diagnosis <- "PS"
df_td_test_F$diagnosis <- "TD"
df_both_F <- rbind(df_ps_F, df_td_test_F)
...
real_pred_lasso_F <- ggplot(data=df_both_F, aes(real_age, pred_age_lasso, color=diagnosis)) +
	...
	labs(title = "Female (Lasso): Gray Matter Density ROIs", subtitle = substring_lasso_F) + 
...
real_pred_h2o_F <- ggplot(data=df_both_F, aes(real_age, pred_age_h2o, color=diagnosis)) +
	geom_point(shape = 16, size = 3, show.legend = TRUE) + theme_minimal() +
	xlim(5,25) + ylim(5,25) + geom_abline() +
	labs(title = "Female (H2O): Gray Matter Density ROIs", subtitle = substring_h2o_F) + 
	xlab("Real Age") + ylab("Predicted Age") +
	geom_smooth(se = TRUE, method = "gam", formula = y ~ s(log(x))) + 
	theme(plot.title = element_text(family="Times", face="bold", size=20))
...
bar_lasso_F <- ggplot(data=summary_lasso_F, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	ggtitle("Female (Lasso): Gray Matter Density ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")

bar_h2o_F <- ggplot(data=summary_h2o_F, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	geom_bar(stat="identity", position=position_dodge()) +
	geom_text(aes(label=N), vjust=1.6, color="white", position = position_dodge(0.9), size=2.5) +
	scale_fill_brewer(palette="Paired") + ylim(c(0,100)) +
	ggtitle("Female (H2O): Gray Matter Density ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")
...
df_M <- df[df$sex == 2, c("goassessDxpmr7", xvars, yvar)]
...
td_M <- df_M[df_M$goassessDxpmr7 == "TD",]
train <- sample(1:nrow(td_M), nrow(td_M)/2, replace=FALSE)

x_td_train_M <- td_M[train, xvars]
x_td_test_M <- td_M[-train, xvars]
x_td_train_M <- as.matrix(x_td_train_M)
x_td_test_M <- as.matrix(x_td_test_M)
y_td_train_M <- td_M[train, yvar]
y_td_test_M <- td_M[-train, yvar]
x_ps_M <- df_M[df_M$goassessDxpmr7 == "PS", xvars]
x_ps_M <- as.matrix(x_ps_M)
y_ps_M <- df_M[df_M$goassessDxpmr7 == "PS", yvar]

# Create heatmap of correlations between potential predictors
cormat <- round(cor(x_td_train_M),2)
...
# Build models
#H20
xy_td_train_M <- cbind(x_td_train_M, y_td_train_M)
colnames(xy_td_train_M)[119] <- "ageAtScan1"
xy_td_test_M <- cbind(x_td_test_M, y_td_test_M)
colnames(xy_td_test_M)[119] <- "ageAtScan1"
xy_ps_M <- cbind(x_ps_M, y_ps_M)
colnames(xy_ps_M)[119] <- "ageAtScan1"
h2o_td_train_M <- as.h2o(xy_td_train_M)
h2o_td_test_M <- as.h2o(xy_td_test_M)
h2o_ps_M <- as.h2o(xy_ps_M)
mod <- h2o.automl(x = xvars, y = "ageAtScan1", training_frame = h2o_td_train_M, max_runtime_secs = 30)
ML_td_test_M <- as.matrix(h2o.predict(mod@leader,h2o_td_test_M))
ML_ps_M <- as.matrix(h2o.predict(mod@leader,h2o_ps_M))

#Lasso
mod_td_M <- glmnet(x_td_train_M, y_td_train_M, family=c("gaussian"))
pdf(NULL)
dev.control(displaylist="enable")
plot(mod_td_M)
...
cv.out_M <- cv.glmnet(x_td_train_M, y_td_train_M, alpha=1)
pdf(NULL)
dev.control(displaylist="enable")
plot(cv.out_M)
...
# predict td
pred_td_test_M <- predict(mod_td_M,s=bestlam_1se,newx=x_td_test_M)
...
df_td_test_M <- data.frame(x_td_test_M)
df_td_test_M$real_age <- y_td_test_M/12
df_td_test_M$pred_age_lasso <- pred_td_test_M/12
df_td_test_M$pred_age_h2o <- ML_td_test_M/12

# Plot Percent Predicted to be over 18 in each two-year age bins, split by diagnosis
df_ps_M$diagnosis <- "PS"
df_td_test_M$diagnosis <- "TD"
df_both_M <- rbind(df_ps_M, df_td_test_M)
...
real_pred_lasso_M <- ggplot(data=df_both_M, aes(real_age, pred_age_lasso, color=diagnosis)) +
	geom_point(shape = 16, size = 3, show.legend = TRUE) + theme_minimal() +
	xlim(5,25) + ylim(5,25) + geom_abline() +
	labs(title = "Male (Lasso): Gray Matter Density ROIs", subtitle = substring_lasso_M) + 
	xlab("Real Age") + ylab("Predicted Age") +
	geom_smooth(se = TRUE, method = "gam", formula = y ~ s(log(x))) + 
	theme(plot.title = element_text(family="Times", face="bold", size=20))
...
real_pred_h2o_M <- ggplot(data=df_both_M, aes(real_age, pred_age_h2o, color=diagnosis)) +
	geom_point(shape = 16, size = 3, show.legend = TRUE) + theme_minimal() +
	xlim(5,25) + ylim(5,25) + geom_abline() +
	labs(title = "Male (H2O): Gray Matter Density ROIs", subtitle = substring_h2o_M) + 
	xlab("Real Age") + ylab("Predicted Age") +
	geom_smooth(se = TRUE, method = "gam", formula = y ~ s(log(x))) + 
	theme(plot.title = element_text(family="Times", face="bold", size=20))
...
bar_lasso_M <- ggplot(data=summary_lasso_M, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	geom_text(aes(label=N), vjust=1.6, color="white", position = position_dodge(0.9), size=2.5) +
	scale_fill_brewer(palette="Paired") + ylim(c(0,100)) +
	ggtitle("Male (Lasso): Gray Matter Density ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")

bar_h2o_M <- ggplot(data=summary_h2o_M, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	geom_bar(stat="identity", position=position_dodge()) +
	geom_text(aes(label=N), vjust=1.6, color="white", position = position_dodge(0.9), size=2.5) +
	scale_fill_brewer(palette="Paired") + ylim(c(0,100)) +
	ggtitle("Male (H2O): Gray Matter Density ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")



# Save plots
pdf(file="/home/butellyn/age_prediction/plots/gmd_TDtest_LassoH2O.pdf", width=12, height=6)
