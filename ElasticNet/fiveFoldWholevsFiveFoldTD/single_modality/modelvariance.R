...
df_both_F$absdiff_realpred_age <- abs(df_both_F$real_age - df_both_F$pred_age)
ks_results <- ks.test(df_both_F[df_both_F$diagnosis == "TD", "absdiff_realpred_age"], df_both_F[df_both_F$diagnosis == "PS", "absdiff_realpred_age"])

# Plot distributions of differences between real and predicted age, colored by diagnosis
ks_string_F <- paste0("KS Test: D = ", round(ks_results$statistic[[1]], digits=2), ", p = ", round(ks_results$p.value[[1]], digits=2))

absdiff_hist_TD_F <- ggplot(data=df_both_F[df_both_F$diagnosis == "TD",], aes(x=absdiff_realpred_age)) +
	geom_histogram(bins=20, alpha=0.5, color="thistle4", fill="thistle1") + xlab("abs(Real - Predicted Age)") +
	ggtitle("Female TDs: Histogram of abs(Real-Predicted Age)") +
	theme_minimal() + labs(subtitle=ks_string_F) + 
	theme(plot.title = element_text(family="Times", face="bold", size=18)) 

absdiff_hist_PS_F <- ggplot(data=df_both_F[df_both_F$diagnosis == "PS",], aes(x=absdiff_realpred_age)) +
	geom_histogram(bins=20, alpha=0.5, color="steelblue4", fill="steelblue1") + xlab("abs(Real - Predicted Age)") +
	ggtitle("Female PSs: Histogram of abs(Real-Predicted Age)") +
	theme_minimal() + labs(subtitle=ks_string_F) + 
	theme(plot.title = element_text(family="Times", face="bold", size=18)) 
...
# Test if the variances in these distributions are significantly different (KS sphericity test)
df_both_M$absdiff_realpred_age <- abs(df_both_M$real_age - df_both_M$pred_age)
ks_results <- ks.test(df_both_M[df_both_M$diagnosis == "TD", "absdiff_realpred_age"], df_both_M[df_both_M$diagnosis == "PS", "absdiff_realpred_age"])

# Plot distributions of differences between real and predicted age, colored by diagnosis
ks_string_M <- paste0("KS Test: D = ", round(ks_results$statistic[[1]], digits=2), ", p = ", round(ks_results$p.value[[1]], digits=2))

absdiff_hist_TD_M <- ggplot(data=df_both_M[df_both_M$diagnosis == "TD",], aes(x=absdiff_realpred_age)) +
	geom_histogram(bins=20, alpha=0.5, color="thistle4", fill="thistle1") + xlab("abs(Real - Predicted Age)") +
	ggtitle("Male TDs: Histogram of abs(Real-Predicted Age)") +
	theme_minimal() + labs(subtitle=ks_string_M) + 
	theme(plot.title = element_text(family="Times", face="bold", size=18)) 

absdiff_hist_PS_M <- ggplot(data=df_both_M[df_both_M$diagnosis == "PS",], aes(x=absdiff_realpred_age)) +
	geom_histogram(bins=20, alpha=0.5, color="steelblue4", fill="steelblue1") + xlab("abs(Real - Predicted Age)") +
	ggtitle("Male PSs: Histogram of abs(Real-Predicted Age)") +
	theme_minimal() + labs(subtitle=ks_string_M) + 
	theme(plot.title = element_text(family="Times", face="bold", size=18)) 
...
##### Save plots
#Elastic Net Plots
pdf(file=paste0("/home/butellyn/age_prediction/plots/", type, "_fiveFoldWholeTDVariance_ElasticNet.pdf"), width=12, height=6)
plot(0:10, type = "n", xaxt="n", yaxt="n", bty="n", xlab = "", ylab = "")
text(5, 8, paste0("KS Tests for ", fullname, " Elastic Net Model"))
grid.arrange(absdiff_hist_TD_F, absdiff_hist_PS_F, ncol=2)
grid.arrange(absdiff_hist_TD_M, absdiff_hist_PS_M, ncol=2)
dev.off()
