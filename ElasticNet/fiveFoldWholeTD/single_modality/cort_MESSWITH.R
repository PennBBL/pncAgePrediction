...
for (i in 1:5) {
	...
	ageAtScan1 <- y_train_input - mean(y_train_input) #March 8, 2019: Outcome centered
	...
	y_test_predicted <- predict(elastic_net_model, x_test_df) + mean(y_test_df$ageAtScan1) #March 8, 2019: Mean added back in
...
ageAtScan1 <- td_F[,yvar] - mean(td_F[,yvar]) #March 8, 2019: Outcome centered
#train_control <- trainControl(method = "repeatedcv", number = 5, repeats = 5, search = "random", verboseIter = TRUE)
train_control <- trainControl(method = "cv")
...
y_ps_predicted <- predict(elastic_net_model, x_ps_input) + mean(ageAtScan1) #March 8, 2019: Mean added back in
