...
folds5 <- createFolds(ps_F$ageAtScan1, k = 5, list = TRUE)
...
for (i in 1:5) {
	x_train_df <- get(paste0("x_ps_train_F_", i))
	x_train_input <- x_train_df[,xvars]
	y_train_df <- get(paste0("y_ps_train_F_", i))
	...
	assign(paste0("mod_ps_F_", i), elastic_net_model)
	x_test_df <- get(paste0("x_ps_test_F_", i))
	y_test_df <- get(paste0("y_ps_test_F_", i))
...
x_ps_train_input <- ps_F[,xvars]
ageAtScan1 <- ps_F[,yvar]
train_control <- trainControl(method = "repeatedcv", number = 5, repeats = 5, search = "random", verboseIter = TRUE)
elastic_net_model <- train(ageAtScan1 ~ . , data = cbind(ageAtScan1, x_ps_train_input), method = "glmnet", 
				preProcess = c("center", "scale"), tuneLength = 25, trControl = train_control) 


# predict on TD's
x_td_F <- df_F[df_F$psTerminal == "TD", c("bblid", xvars)]
y_td_F <- df_F[df_F$psTerminal == "TD", c("bblid", yvar)]
x_td_input <- x_td_F[,xvars]
y_td_input <- y_td_F[,yvar]
ageAtScan1 <- y_td_input
y_td_predicted <- predict(elastic_net_model, x_td_input)

# create final dfs
df_ps_F <- predicted_values
df_td_F <- merge(x_td_F, y_td_F)
df_td_F$predicted_ageAtScan1 <- y_td_predicted
...
real_pred_F <- ggplot(data=df_both_F, aes(real_age, pred_age, color=diagnosis)) +
	...
	labs(title = "Female (El5 PS Model): Cortical Thickness ROIs", subtitle = substring_F) + 
...
bar_F <- ggplot(data=summary_F, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	ggtitle("Female (El5 PS Model): Cortical Thickness ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")
...
folds5 <- createFolds(ps_M$ageAtScan1, k = 5, list = TRUE)
...
#####Elastic Net: out of sample predictions for TD's
for (i in 1:5) {
	x_train_df <- get(paste0("x_ps_train_M_", i))
	x_train_input <- x_train_df[,xvars]
	y_train_df <- get(paste0("y_ps_train_M_", i))
	y_train_input <- y_train_df$ageAtScan1
	ageAtScan1 <- y_train_input
	train_control <- trainControl(method = "repeatedcv", number = 5, repeats = 5, search = "random", verboseIter = TRUE)
	elastic_net_model <- train(ageAtScan1 ~ . , data = cbind(ageAtScan1, x_train_input), method = "glmnet", 
				preProcess = c("center", "scale"), tuneLength = 25, trControl = train_control) 
				# just used parameters from here: https://www.datacamp.com/community/tutorials/tutorial-ridge-lasso-elastic-net
	assign(paste0("mod_ps_M_", i), elastic_net_model)
	x_test_df <- get(paste0("x_ps_test_M_", i))
	y_test_df <- get(paste0("y_ps_test_M_", i))
	y_test_predicted <- predict(elastic_net_model, x_test_df)
	if (i == 1) { predicted_values <- merge(cbind(x_test_df, y_test_predicted), y_test_df)
	} else { predicted_values <- rbind(predicted_values, merge(cbind(x_test_df, y_test_predicted), y_test_df))
	}
...
#####Elastic Net: predictions for PS's 
# train on full TD set
x_ps_train_input <- ps_M[,xvars]
ageAtScan1 <- ps_M[,yvar]
train_control <- trainControl(method = "repeatedcv", number = 5, repeats = 5, search = "random", verboseIter = TRUE)
elastic_net_model <- train(ageAtScan1 ~ . , data = cbind(ageAtScan1, x_ps_train_input), method = "glmnet", 
				preProcess = c("center", "scale"), tuneLength = 25, trControl = train_control) 
...
# predict on TD's
x_td_M <- df_M[df_M$psTerminal == "TD", c("bblid", xvars)]
y_td_M <- df_M[df_M$psTerminal == "TD", c("bblid", yvar)]
x_td_input <- x_td_M[,xvars]
y_td_input <- y_td_M[,yvar]
ageAtScan1 <- y_td_input
y_td_predicted <- predict(elastic_net_model, x_td_input)

# create final dfs
df_ps_M <- predicted_values
df_td_M <- merge(x_td_M, y_td_M)
df_td_M$predicted_ageAtScan1 <- y_td_predicted
...
real_pred_M <- ggplot(data=df_both_M, aes(real_age, pred_age, color=diagnosis)) +
	geom_point(shape = 16, size = 3, show.legend = TRUE) + theme_minimal() +
	xlim(5,25) + ylim(5,25) + geom_abline() +
	labs(title = "Male (El5 PS Model): Cortical Thickness ROIs", subtitle = substring_M) + 
	xlab("Real Age") + ylab("Predicted Age") + geom_smooth(se = TRUE, method = "lm") +
	theme(plot.title = element_text(family="Times", face="bold", size=18)) 
...
bar_M <- ggplot(data=summary_M, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	geom_text(aes(label=N), vjust=1.6, color="white", position = position_dodge(0.9), size=2.5) +
	scale_fill_brewer(palette="Paired") + ylim(c(0,100)) +
	ggtitle("Male (El5 PS Model): Cortical Thickness ROIs") + xlab("Age Category") + ylab("Predicted Adult (%)")


# Save plots
pdf(file="/home/butellyn/age_prediction/plots/cort_fiveFoldWholePS_ElasticNet.pdf", width=12, height=6)
