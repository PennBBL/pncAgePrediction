...
corr_df <- as.data.frame(matrix(0, nrow=837, ncol=6))
colnames(corr_df) <- c("ROI", "Modality", "TD_F_Corr", "PS_F_Corr", "TD_M_Corr", "PS_M_Corr")
corr_df$ROI <- colnames(td_F)[5:841]
...
for (i in 5:841) {
	corr_df[i-4,3] <- cor(td_F[,i],td_F$ageAtScan1)
	corr_df[i-4,4] <- cor(ps_F[,i],ps_F$ageAtScan1)
	corr_df[i-4,5] <- cor(td_M[,i],td_M$ageAtScan1)
	corr_df[i-4,6] <- cor(ps_M[,i],ps_M$ageAtScan1)
}
