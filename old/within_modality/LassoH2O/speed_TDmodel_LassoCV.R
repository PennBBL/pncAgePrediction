...
# Load data
df1 <- read.csv("/home/butellyn/age_prediction/data/n9498_cnb_zscores_NON-AGE-REGRESSED.csv", header=T)
df2 <- read.csv("/home/butellyn/age_prediction/data/n9498_demographics_go1_20161212.csv", header=T)
df3 <- read.csv("/home/butellyn/age_prediction/data/n9498_diagnosis_dxpmr.csv", header=T)
df <- merge(df1, df2)
df <- merge(df, df3)
...
xvars <- c("abf_s_z", "att_s_z", "wm_s_z", "vmem_s_z", "fmem_s_z", "smem_s_z", "lan_s_z", "nvr_s_z", "spa_s_z", "eid_s_z", "edi_s_z", "adi_s_z")

#xvars <- c("abf_a_z", "att_a_z", "wm_a_z", "vmem_a_z", "fmem_a_z", "smem_a_z", "lan_a_z", "nvr_a_z", "spa_a_z", "eid_a_z", "edi_a_z", "adi_a_z",
#	"abf_a_z_sq", "att_a_z_sq", "wm_a_z_sq", "vmem_a_z_sq", "fmem_a_z_sq", "smem_a_z_sq", "lan_a_z_sq", "nvr_a_z_sq", "spa_a_z_sq", "eid_a_z_sq", 
#	"edi_a_z_sq", "adi_a_z_sq", "abf_a_z_cu", "att_a_z_cu", "wm_a_z_cu", "vmem_a_z_cu", "fmem_a_z_cu", "smem_a_z_cu", "lan_a_z_cu", "nvr_a_z_cu",
#	"spa_a_z_cu", "eid_a_z_cu", "edi_a_z_cu", "adi_a_z_cu")

############################ Females ############################

df_F <- df[df$sex == 2, c("DXPMR7", xvars, yvar)]
...
x_td_F <- df_F[df_F$DXPMR7 == "TD", xvars]
x_td_F <- as.matrix(x_td_F)
y_td_F <- df_F[df_F$DXPMR7 == "TD", yvar]
x_ps_F <- df_F[df_F$DXPMR7 == "PS", xvars]
x_ps_F <- as.matrix(x_ps_F)
y_ps_F <- df_F[df_F$DXPMR7 == "PS", yvar]
...
ps_real_pred_F <- ggplot(data=df_ps_F, aes(real_age, pred_age, color=pc)) +
	geom_point(shape = 16, size = 2, show.legend = FALSE) + theme_minimal() +
	scale_color_gradient(low = "#0091ff", high = "#f0650e") + xlim(5,24) + ylim(5,24) + geom_abline() +
	ggtitle("PS Female: Speed Cognitive Age") + 
...
td_real_pred_F <- ggplot(data=df_td_F, aes(real_age, pred_age, color=pc)) +
	geom_point(shape = 16, size = 2, show.legend = FALSE) + theme_minimal() +
	scale_color_gradient(low = "#0091ff", high = "#f0650e") + xlim(5,24) + ylim(5,24) + geom_abline() +
	ggtitle("TD Female: Speed Cognitive Age") + 
	xlab("Real Age") + ylab("Predicted Age")
...
bar_F <- ggplot(data=summary_both_F, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	ggtitle("Female Cognitive Age: Speed") + xlab("Age Category") + ylab("Predicted Adult (%)")

############################ Males ############################

df_M <- df[df$sex == 1, c("DXPMR7", xvars, yvar)]
...
x_td_M <- df_M[df_M$DXPMR7 == "TD", xvars]
x_td_M <- as.matrix(x_td_M)
y_td_M <- df_M[df_M$DXPMR7 == "TD", yvar]
x_ps_M <- df_M[df_M$DXPMR7 == "PS", xvars]
x_ps_M <- as.matrix(x_ps_M)
y_ps_M <- df_M[df_M$DXPMR7 == "PS", yvar]
...
ps_real_pred_M <- ggplot(data=df_ps_M, aes(real_age, pred_age, color=pc)) +
	geom_point(shape = 16, size = 2, show.legend = FALSE) + theme_minimal() +
	scale_color_gradient(low = "#0091ff", high = "#f0650e") + xlim(5,24) + ylim(5,24) + geom_abline() +
	ggtitle("PS Male: Speed Cognitive Age") + 
	xlab("Real Age") + ylab("Predicted Age")
...
td_real_pred_M <- ggplot(data=df_td_M, aes(real_age, pred_age, color=pc)) +
	geom_point(shape = 16, size = 2, show.legend = FALSE) + theme_minimal() +
	scale_color_gradient(low = "#0091ff", high = "#f0650e") + xlim(5,24) + ylim(5,24) + geom_abline() +
	ggtitle("TD Male: Speed Cognitive Age") + 
	xlab("Real Age") + ylab("Predicted Age")
...
bar_M <- ggplot(data=summary_both_M, aes(x=Age_Category, y=Percent_Pred_Adult, fill=Diagnosis)) +
	...
	geom_text(aes(label=N), vjust=1.6, color="white", position = position_dodge(0.9), size=3.5) +
	scale_fill_brewer(palette="Paired") + ylim(c(0,100)) +
	ggtitle("Male Cognitive Age: Speed") + xlab("Age Category") + ylab("Predicted Adult (%)")



# Save plots
pdf(file="/home/butellyn/age_prediction/plots/speed_TDmodel_LassoCV.pdf", width=8, height=6)
